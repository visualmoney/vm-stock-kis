...
### 부수 관찰 — 의존 간선 수가 428 → 359 로 줄었습니다

모듈 수는 92로 같은데 간선이 줄었습니다. `root_packages` 로 겹치는 루트를 여러
개 주면 같은 import 가 중복 계상되던 것으로 보입니다. **계약이 약해진 것은
아닙니다** — 고의 위반 2종을 A 구성에서 다시 넣어 둘 다 `BROKEN` 을 확인했습니다.
...
lint-imports                          Analyzed 92 files, 359 dependencies. 2 kept, 0 broken
coverage                              92%  (게이트 90)
